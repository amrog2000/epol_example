...
//      CComLog::instance().log(m_strLogMsg);"[SERVER] writetask %d sending %d : [%d] %s\n", pthread_self(), rdata->fd, rdata->n_size, rdata->line);
        ...
        if ((n = send(rdata->fd, rdata->line, rdata->n_size, 0)) < 0)
        {
            if (errno == ECONNRESET)
                close(rdata->fd);
            CComLog::instance().log(m_strLogMsg);"[SERVER] Error: send responce failed: %s\n", strerror(errno));
	    m_strLogMsg = "[SERVER] Error: send responce failed: " + strerror(errno);
    	    CComLog::instance().log(m_strLogMsg, CComLog::Error);
        }
        else if (n == 0)
        {
            close(rdata->fd);
            CComLog::instance().log(m_strLogMsg);"[SERVER] Error: client closed connection.");
        }
        else
        {
...
//********************************************************************************************//
double CEpoll::Get_CPU_Time(void)  // incomplete method....
{
    double user, sys;
    ...
    CComLog::instance().log(m_strLogMsg);"[SERVER] user time=%g, sys time=%g\n", user, sys); 
    return sys;
}
//********************************************************************************************//
